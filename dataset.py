...
class HockeyFightDataset(Dataset):
    """Video dataset."""
    ...
    def __init__(self, root_dir, spatial_transform_rgb, spatial_transform_flow, 
        ...
        for class_path in file_list:
            ...
            for video_path in glob.glob(class_path + "\*_xvid"):
    ...
    def __getitem__(self, idx):
        ...
        if self.input_type == 'rgb':
            inpSeq = []
            for i in range(1, num_jpg_files + 1, self.stride):
                if not (i > num_jpg_files):
                    if len(inpSeq) == self.numFrames:
                        break
                    fl_name = video_path + '/' + 'image_' + str(i).zfill(5) + '.jpg'
