...
from dataset import HockeyFightDataset
...
numFrames = 10
stride = 3
input_type = 'rgb_flow_combined'  # flow, rgb, rgb_flow_combined (if rgb_flow_combined => flow_type=raw_flow)
flow_type = 'raw_flow'  # raw_flow, flow_mag_or, flow_dot_product
fusion_method = 'late' # early, late
method = 'squeeze'  # loop, squeeze
cnn_lstm_model = 'vgg19' # vgg19, resnet50
train_path = './dataset/train'
test_path = './dataset/test'
log_dir = 'logs_final/logs6/input_type = rgb_flow_combined, fusion_method=late'
...
train_dataset = HockeyFightDataset(train_path, spatial_transform_rgb=spatial_transform_rgb, spatial_transform_flow=spatial_transform_flow, numFrames=numFrames, stride=stride, image_size=image_size,
                                   input_type=input_type, flow_type=flow_type, fusion_method=fusion_method)
val_dataset = HockeyFightDataset(test_path, spatial_transform_rgb=spatial_transform_rgb, spatial_transform_flow=spatial_transform_flow, numFrames=numFrames, stride=stride, image_size=image_size,
                                   input_type=input_type, flow_type=flow_type, fusion_method=fusion_method)
...
train_count = len(glob.glob(train_path+'\**\*_xvid'))
test_count = len(glob.glob(test_path+'\**\*_xvid'))
